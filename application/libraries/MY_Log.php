...
// ------------------------------------------------------------------------
    ...
    class MY_Log extends CI_Log
    {
        ...
        protected $_levels = array('ERROR' => '1', 'DEBUG' => '2', 'INFO' => '3', 'ALL' => '4', 'EMAIL' => '5', 'SABRE' => '6','AFFILIATE' => '7','MY_DEBUG_LOG'=>'8');
        ...
        public function write_log($level = 'error', $msg, $php_error = FALSE)
        {
            ...
            elseif ($level == 'EMAIL')
            {
                $filepath = $this->_log_path . 'email-log-' . date('Y-m-d') . '.php';
            }
            elseif ($level == 'SABRE')
            {
                $filepath = $this->_log_path . 'sabre-log-' . date('Y-m-d') . '.php';
            }
            else if ($level == 'AFFILIATE')
            {
                $filepath = $this->_log_path . 'affiliate-log-' . date('Y-m-d') . '.php';
            }
            else if ($level == 'MY_DEBUG_LOG'){
                $filepath = $this->_log_path . 'my-debug-log-' . date('Y-m-d') . '.php';
            }
            else
            {
            ...
            $message .= $level . ' ' . (($level == 'INFO') ? ' -' : '-') . ' ' . date($this->_date_fmt) . ' --> ' . print_r($msg, true) . "\n";
